...
import 'package:novinpay/dialogs/satna_transfer_confirm_dialog.dart';
...
class SatnaMoneyTransfer extends StatefulWidget {
  @override
  _SatnaMoneyTransferMoneyTransferState createState() =>
      _SatnaMoneyTransferMoneyTransferState();
}

class _SatnaMoneyTransferMoneyTransferState extends State<SatnaMoneyTransfer>
  ...
  final _inputTextFieldFamily = TextEditingController();
  ...
  String _familyValidator(String value) {
    if (value == null || value.isEmpty) {
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Space(2),
                  TextFormField(
                    controller: _inputTextFieldFamily,
                    validator: _familyValidator,
                    decoration: InputDecoration(
                      labelText: 'نام خانوادگی صاحب حساب',
                      counterText: '',
                    ),
                  ),
                  Space(2),
  ...
  void _submit() async {
    ...
    final family = _inputTextFieldFamily.text;
    final result = await showCustomBottomSheet<bool>(
      context: context,
      title: Text('انتقال وجه ساتنا'),
      child: SatnaTransferConfirmDialog(
        name: name,
        source: source,
    ...
    final data = await Loading.run(
      context,
      nh.boomMarket.satnaTransfer(
        sourceDepositNumber: source,
        destinationIbanNumber: destination,
        receiverName: name,
    ...
    if (data.content.status != true) {
      await showFailedTransaction(context, data.content.description, null, null,
          'انتقال وجه ساتنا', amount);
      return;
    }
    ...
    await Navigator.of(context).pushReplacement(
      MaterialPageRoute(
        builder: (context) => OverViewPage(
          ...
          rrn: '',
          topupPin: null,
          description: 'انتقال وجه ساتنا به شماره شبا: $destination',
          amount: amount,
          isShaparak: false,
        ),
      ),

...
mixin PhoneSelectionMixin<T extends StatefulWidget> on MruSelectionMixin<T> {
  void _getContact() async {
    ...
    final m = phones[0].value.replaceAll('+98', '0').replaceAll(' ', '');
    ;
  ...
  Future<MruItem> showPhoneSelectionDialog(
          {Widget title = const Text('تلفن های من')}) =>
      showMruSelectionDialog(
        mru: MruType.landPhone,
        title: isWeb
            ? title
            : Row(
